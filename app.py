...
def index():
    ...
    if 'username' in session:
        return render_template('index.html', username=session['username'], now=datetime.now())
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password'], password):
            ...
            return redirect(url_for('index')) # Redirige a la página de home después del login
        else:
...
@app.route('/nosotros')
def nosotros():
    """Sirve tu archivo contacto.html original."""
    username = session.get('username')
    return render_template('nosotros.html', username=username)
